...
【8】公司的概念，德鲁克，机械工业出版社。"#,
  ...
  let mut ratio_count = HashMap::new();
...
{:>w$} : {:>w2$}"#,
    ...
    macro_rules! compress_ratio {
      ($tip:expr, $func:expr) => {{
        ...
        let ratio = 100.0 * (len as f64) / (txt_len as f64);
        ratio_count.insert($tip, ratio + ratio_count.get($tip).unwrap_or(&0.0));
  ...
  let len = li.len() as f64;
  let mut ratio_li = ratio_count.iter().collect::<Vec<_>>();
  ratio_li.sort_by(|a, b| b.1.partial_cmp(a.1).unwrap());
  for (k, v) in ratio_li {
    println!("{}\t{:.2}%", k, v / len);
  }
